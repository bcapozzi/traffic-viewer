...
Elm.ResourceDecoder.make = function (_elm) {
   ...
   var TimedCount = F2(function (a,b) {
      return {t: a,count: b};
   });
   var timedCountDecoder = A3($Json$Decode.object2,
   TimedCount,
   A2($Json$Decode._op[":="],"t",$Json$Decode.$int),
   A2($Json$Decode._op[":="],"count",$Json$Decode.$int));
   var resourceCountDecoder = A3($Json$Decode.object2,
   ResourceCount,
   A2($Json$Decode._op[":="],"id",$Json$Decode.string),
   A2($Json$Decode._op[":="],
   "counts",
   $Json$Decode.list(timedCountDecoder)));
   var countDecoderColl = A2($Json$Decode.object1,
   $Basics.identity,
   A2($Json$Decode._op[":="],
   "resourceCounts",
   $Json$Decode.list(resourceCountDecoder)));
   ...
   var viewResources = function (maybeResources) {
      ...
      var viewResource = function (resource) {
         ...
         "ID: ",
         A2($Basics._op["++"],resource.id,"-->"))),
         A2($List.map,viewCoord,resource.coords)));
      };
      var _p0 = maybeResources;
      if (_p0.ctor === "Nothing") {
            return A2($Html.div,
            _U.list([]),
            _U.list([$Html.text("No resources to display. Try clicking the button")]));
         } else {
            return A2($Html.ul,
            _U.list([]),
            A2($List.map,viewResource,_p0._0));
         }
   };
   var viewResourceCounts = function (maybeResourceCounts) {
      var _p1 = maybeResourceCounts;
      if (_p1.ctor === "Nothing") {
            return A2($Html.div,
            _U.list([]),
            _U.list([$Html.text("No resource counts to display.  Try clicking the button")]));
         } else {
            return A2($Html.div,
            _U.list([]),
            ...
            "Got ",
            A2($Basics._op["++"],
            $Basics.toString($List.length(_p1._0)),
   ...
   var view = F2(function (address,model) {
      ...
      _U.list([A2($Html.button,
              _U.list([A2($Html$Events.onClick,address,GetResources)]),
              _U.list([$Html.text("Click to get resources!")]))
              ,A2($Html.button,
   ...
   var update = F2(function (action,model) {
      ...
      {case "NoOp": return {ctor: "_Tuple2"
         ...
         case "GetResources": return {ctor: "_Tuple2"
                                     ...
                                     ,_1: getResources};
         case "GetResourceCounts": return {ctor: "_Tuple2"
